...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Override port if PORT environment variable is set (common in cloud platforms like Render)
	if portStr := os.Getenv("PORT"); portStr != "" {
		if p, err := strconv.Atoi(portStr); err == nil && p > 0 && p <= 65535 {
			cfg.Port = p
			log.Infof("overriding port from environment variable PORT: %d", cfg.Port)
		}
	}
